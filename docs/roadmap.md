...
- [ ] **Gathering POIs + lockpicking + minigames** (player-drafted; keep light — `draft_enviornment.txt` resource tables, `checks-design.md` hard-vs-soft checks, `environment-poi-system.md` "utility POIs", `skills.json` maps Mining/Woodcutting→Athletics, Fishing→Survival, Lockpicking→Thieving; tools `pick-miners.json` + `thieves-kit.json` exist, **fishing tool doesn't**):
  - [x] **Gathering-node POIs (v1)** ✅ `d4378eb` — `ironvein-seam` (pickaxe→athletics), `still-waters-fishing` (fishing-rod→survival), `buried-strongbox` (thieves-kit→thieving, +athletics force-open). Tool-gated `choice` (option only appears if you carry the tool) → `check` → tiered `loot`, looping so you can keep working the node. + 6 items: iron/copper ore, rough gem, raw/large fish, fishing rod.
  - [x] **Tiered gathering yields** ✅ `d4378eb` — `grantGuaranteedLoot`→`grantLoot`: guaranteed still always drops, then rolls `Rolls`× against weighted `Tiers` (weighted entry per roll), and quantities accept a `[min,max]` range. Tested.
  - [x] **Lockpicking** ✅ `d4378eb` — `buried-strongbox` (thieving check gated by thieves-kit, with an Athletics force-open alt that yields less). Zero new mechanics, as planned.
  - [ ] **Minigames (mining / fishing / lockpicking)** — ⬅ **the one remaining M5.5 sub-piece.** The gathering/lockpicking *mechanics* are now playable via the standard check UI; the minigame is the interactive skin on top. v1 = **skin-over-check**, and the anti-cheat model dictates the shape: the server `check` node must stay authoritative (a client-decided minigame is trivially cheatable), so the flow is (1) player picks the work option → (2) server resolves the check and returns the result + a `minigame` hint on the StepResult → (3) `poiExplore.js` intercepts that hint and plays a per-game overlay (lock tumblers / cast-and-reel / ore-swing timing bar) that *lands on the server's already-decided outcome* → (4) reveal the loot. Needs: a `minigame` field on `POIStep` (server passes it through untouched), an overlay module per game (mirrors the combat handoff), and a text-speed-style tunable in settings. Deliberately a focused frontend slice, not crammed into this backend pass — build it against the three POIs above. *(S skin-over-check → M–L if ever promoted to a first-class node type.)*
